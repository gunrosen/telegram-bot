...
const func = async () => {
   ...
   */
  const chat = await checkService.getBot().getChat(-1001510597701)
  const admin = await checkService.getBot().getChatAdministrators(-1001510597701)
  const number = await checkService.getBot().getChatMembersCount(-1001510597701)
  console.log(chat)
  console.log(admin)
  console.log(number)
  ...
  checkService.getBot().on('new_chat_members', (msg: any) => {
    console.log("new_chat_members");
  })
  checkService.getBot().on('new_chat_photo', (msg: any) => {
    console.log("new_chat_photo");
  })
  checkService.getBot().on('pinned_message', (msg: any) => {
    console.log("pinned_message");
  })
  checkService.getBot().on('poll', (msg: any) => {
    console.log("poll");
  })
  checkService.getBot().on('contact', (msg: any) => {
    console.log("contact");
  })
}
